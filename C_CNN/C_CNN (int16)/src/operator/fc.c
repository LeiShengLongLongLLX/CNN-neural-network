...
void FullyConnected(const Tensor* input, const Tensor* weights, const int16_t* bias, Tensor* output)
{
    int N = input->N;      // 批量大小
    ...
    for (int n = 0; n < N; n++)    // 遍历批量
    {
        for (int o = 0; o < output_features; o++) // 遍历输出向量元素
        {
            ...
            {
                int16_t v_in = input->data[IDX4(n, 0, 0, i, 1, 1, input_features)];
                int16_t w = weights->data[IDX4(o, 0, 0, i, 1, 1, input_features)];
                sum += (int32_t)v_in * (int32_t)w;
            }
            if (bias != NULL)  // 加上偏置
                sum += bias[o];
            
            output->data[IDX4(n, 0, 0, o, 1, 1, output_features)] = saturate_int16_i32(sum); // 存储输出向量
        }
    }
}
