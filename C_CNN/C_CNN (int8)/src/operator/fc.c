...
void FullyConnected(const Tensor* input, const Tensor* weights, const int8_t* bias, Tensor* output)
{
    int N = input->N;      // 批量大小
    ...
    for (int n = 0; n < N; n++)    // 遍历批量
    {
        for (int o = 0; o < output_features; o++) // 遍历输出向量元素
        {
            ...
            {
                int8_t v_in = input->data[IDX4(n, 0, 0, i, 1, 1, input_features)];
                int8_t w = weights->data[IDX4(o, 0, 0, i, 1, 1, input_features)];
