...
// 展平函数
// 将 [N, C, H, W] 展平成 [N, C*H*W, 1, 1]
// input: 输入张量
// output: 输出张量
void Flatten_int32(const Tensor* input, Tensor* output)
{
    int N = input->N;       // 批量大小
    int C = input->C;       // 通道数
    int H = input->H;       // 高度
        ...
        for (int c = 0; c < C; c++)      // 遍历通道
        {
            for (int h = 0; h < H; h++)  // 遍历高度
            {
                for (int w = 0; w < W; w++)  // 遍历宽度
                {
                    ...
                    int out_index = IDX4(n, flat_index, 0, 0, out_features, 1, 1);

                    // input[n][c][h][w] 
                    // output[n][flat_index][0][0]
